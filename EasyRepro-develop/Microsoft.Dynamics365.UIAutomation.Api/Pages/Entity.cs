...
namespace Microsoft.Dynamics365.UIAutomation.Api
{
        ...
        : XrmPage 
    {
        ...
        public void OpenRecordJS(string id, string entity, int thinkTime = Constants.DefaultThinkTime)
        {
            Browser.ThinkTime(thinkTime);

            this.Execute(GetOptions("Get GUID for the Current Record"), driver =>
            {
                SwitchToContentFrame();

                try
                {
                    driver.ExecuteScript("Xrm.Utility.openEntityForm('" + entity + "', '" + id + "');");
                }
                catch (Exception ex)
                {
                    return true;
        ...
        public void SetValueJS(string field, string value, int thinkTime = Constants.DefaultThinkTime)
        {
            Browser.ThinkTime(thinkTime);
            ...
            this.Execute(GetOptions("Get GUID for the Current Record"), driver =>
            {
                ...
                try
                {
                    driver.ExecuteScript("Xrm.Page.getAttribute('" + field + "').setValue(" + value +");");
                }
                catch(Exception ex)
                {
                    return true;
